...
// TODO: thinking about added convenience methods for receiving using the standard HTTP methods.
//
// The idea would be, instead of:
//
//     reqr.Receive(into, Get("users/"), Body(body))
//
// you could do:
//
//     reqr.Get("users/", into, Body(body))
//
// The method signature would be:
//
//     func (r *Requester)Get(path string, into interface{}, opts ...Option)
//
// There would be similar GetContext, Post, PostContext, Delete, DeleteContext, etc.
//
// This prototype code works, but it's unclear whether there is enough value in the
// convenience to expand the API.
//
/*func pushOpt(o Option, opts []Option) []Option {
	opts = append(opts, nil)
	copy(opts[1:], opts)
	opts[0] = o
	return opts
...
func (r *Requester) receiveMethod(ctx context.Context, o1 Option, into interface{}, opts ...Option) (*http.Response, []byte, error) {
	if o, ok := into.(Option); ok {
		opts = pushOpt(o, opts)
		into = nil
	}

	opts = pushOpt(o1, opts)

	if ctx == nil {
		return r.Receive(into, opts...)
	} else {
		return r.ReceiveContext(ctx, into, opts...)
	}
}

func (r *Requester) Get(path string, into interface{}, opts ...Option) (*http.Response, []byte, error) {
	return r.receiveMethod(nil, Get(path), into, opts...)
}

func (r *Requester)GetContext(ctx context.Context, path string, into interface{}, opts ...Option) (*http.Response, []byte, error) {
	return r.receiveMethod(ctx, Get(path), into, opts...)
}*/
